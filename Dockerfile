...
RUN apt-get update \
    ...
    && printf '#!/bin/sh\necho "== entrypoint diagnostics =="\nls /dev/nvidia* 2>/dev/null || echo "DIAG: no /dev/nvidia* devices"\n[ -f /proc/driver/nvidia/version ] && cat /proc/driver/nvidia/version || echo "DIAG: no nvidia kernel driver"\necho "DIAG: ldconfig libcuda: $(ldconfig -p 2>/dev/null | grep libcuda.so.1 | head -1 || echo none)"\necho "DIAG: NVIDIA_VISIBLE_DEVICES=${NVIDIA_VISIBLE_DEVICES:-unset} NVIDIA_DRIVER_CAPABILITIES=${NVIDIA_DRIVER_CAPABILITIES:-unset}"\nif ! ldconfig -p 2>/dev/null | grep -q "libcuda.so.1"; then\n  export LD_LIBRARY_PATH="/usr/local/cuda/compat:${LD_LIBRARY_PATH:-}"\n  echo "DIAG: host driver not injected, using bundled cuda-compat"\nfi\necho "DIAG: applying OrcaBonsai abliteration LoRA (alpha=${LORA_ALPHA:-1}) via entrypoint"\nLORA_ARGS="--jinja --chat-template-file /opt/prism/chat-template.jinja --lora /opt/prism/bonsai-abliterate-lora.gguf"\nif [ "${LORA_ALPHA:-1}" != "1" ]; then\n  LORA_ARGS="--jinja --chat-template-file /opt/prism/chat-template.jinja --lora-scaled /opt/prism/bonsai-abliterate-lora.gguf:${LORA_ALPHA}"\nfi\nexec /opt/prism/llama-server $LORA_ARGS "$@"\n' > /usr/local/bin/entrypoint.sh \
    && chmod +x /usr/local/bin/entrypoint.sh

# Chat template with reasoning-effort normalization: accepts high/max/unknown
# efforts (mapped to xhigh) instead of raising a Jinja exception (HTTP 500).
COPY chat-template.jinja /opt/prism/chat-template.jinja
